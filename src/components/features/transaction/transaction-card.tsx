 ...
 * 下段 = 左に取引の金額、右に残高の情報（この取引の後に債権がいくら／債務がいくら）。
 *
 * 下段の左右はラベル＋数字という同じ組み方にしてあるので、位置もサイズもそろう。
...
export function TransactionCard({
  ...
  return (
    ...
    >
      ...
      {/* 下段: 左が取引の金額、右が残高。同じ組み方にして行とサイズをそろえる */}
      <div className="flex border-t bg-muted/30">
        <div className="min-w-0 flex-1 px-3 py-1.5">
          <div className="text-[11px] leading-4 text-muted-foreground">金額</div>
          {/* 金額は色を持たせない。向きは名目チップ、債権／債務は残高の色が示す */}
          <div className="truncate text-base font-bold leading-6 tabular-nums text-foreground">
            {formatTransactionAmount(statement.amount)}
          </div>
        </div>
        <div className="min-w-0 flex-1 border-l px-3 py-1.5 text-right">
          <div className="truncate text-[11px] leading-4 text-muted-foreground">
            {balanceRole.balanceLabel}
          </div>
          <div
            className={cn(
              "truncate text-base font-bold leading-6 tabular-nums",
